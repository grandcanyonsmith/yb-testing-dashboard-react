...
const filters = [
  ...
  },
  {
    ...
    name: 'Test Type',
    options: [
      { value: 'uiTests', label: 'UI Tests' },
      { value: 'apiTests', label: 'API Tests' },
    ],
  },
]
          ...
          >
            <Dialog.Panel className="relative w-full max-w-xs h-full ml-auto flex flex-col overflow-y-auto bg-gray-900 py-4 pb-6 shadow-xl text-white">
              ...
              <form className="mt-4 px-4 space-y-6">
                {filters.map((section) => (
                  <Disclosure as="div" key={section.name} className="border-t border-gray-200 py-6">
                    {({ open }) => (
                      <>
                        <h3 className="-mx-2 -my-3 flow-root">
                          <Disclosure.Button className="w-full flex items-center justify-between px-2 py-3 text-sm text-white-50 bg-gray-900">
