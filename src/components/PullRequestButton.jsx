...
const PullRequestButton = ({ code, testName }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('https://j2qqehazvsvt44nt44sdrlx4le0womfr.lambda-url.us-west-2.on.aws/', data);
  ...
  return (
    <>
      <button
        onClick={openModal}
        className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
            ...
            />
                 <div className="mt-4">
                  <label className="inline-flex items-center mt-2">
                    <input type="radio" className="form-radio text-indigo-600" name="commitType" value="pr" checked={commitType === 'pr'} onChange={handleCommitTypeChange} />
                    <span className="ml-2">Create a new branch for this commit and start a pull request</span>
                  </label>
                  {commitType === 'pr' && (
                    <input
                      className="w-full px-3 py-2 mt-2 text-gray-300 border rounded-lg focus:outline-none bg-gray-700 border-gray-600"
