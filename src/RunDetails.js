...
const TestDashboard = () => {
  ...
  const fetchPreviousRuns = async () => {
    ...
    try {
      ...
      setPreviousRunsData(response.data);
        ...
        {selectedRun && (
          <div className="lg:flex lg:items-center lg:justify-between bg-gray-800 px-4 py-5 sm:px-6">
            <div className="min-w-0 flex-1">
              <h2 className="text-2xl font-bold leading-7 text-white sm:truncate sm:text-3xl sm:tracking-tight">{selectedRun.formattedTestName}</h2>
            ...
            <div className="mt-5 flex lg:ml-4 lg:mt-0">
              <button 
                ...
                onClick={() => navigate(`/viewCode?testName=${encodeURIComponent(selectedRun.testName)}`)}
              >
                View Code
              </button>
            </div>
      ...
      <main className="mt-4 px-4 sm:px-0">
        ...
        <section className="mt-8">
          ...
          <div className="w-full p-6 bg-gray-800 shadow rounded-lg">
            <pre><code className="language-markup">{selectedRun && selectedRun.logs.join('\n')}</code></pre>
          </div>
        </section>
        <section className="mt-8">
